...
static int	print_lst(t_plist *lst)
{
	int		cnt_tmp;
	long	cnt;
	t_plist	*tmp;
...
static char	*get_arg(char c, va_list args)
{
	...
	if (c == 'c')
		...
		arg = arg_x_largex(va_arg(args, long), 0);
	else if (c == 'X')
		arg = arg_x_largex(va_arg(args, long), 1);
	else
		arg = arg_c('%');
	return (arg);
}
...
static t_plist	*make_lst(const char *format, va_list args)
{
	int		i;
	int		j;
	t_plist	*res;
